 ...
 * @param revalidateKey
 * @returns Insure you provide Generic RequestType of the Data you want to add and the Generic DataType of the data returned from the API response to get TypeScript intelligence
 */
const useAddData = <RequestType, DataType>(
  ...
  const { mutateAsync, isPending, error, isSuccess, data } = useMutation({
    mutationFn: (data: RequestType) =>
      apiClient.post(data, requestArgs?.config || {}),
    onSuccess: () => {
